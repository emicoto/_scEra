	...
	(function (p, t, h) {
		...
		function e(a) {
				...
				for (l = 0; l < f._callbacks[a].length; ++l) {
					...
					if ((g || !d.seq || n[d.seq] == d.level) && h == d.action) {
						...
						(c = "keypress" == h && !r.metaKey && !r.ctrlKey) ||
							((c = d.modifiers), (c = b.sort().join(",") === c.sort().join(",")));
						c &&
			...
			f._handleKey = function (a, c, e) {
				...
				for (f = 0; f < g.length; ++f)
					g[f].seq
						? g[f].level == l && ((k = !0), (c[g[f].seq] = 1), h(g[f].callback, e, g[f].combo, g[f].seq))
		...
		if (p) {
			...
			e.prototype.stopCallback = function (a, b) {
				return -1 < (" " + b.className + " ").indexOf(" mousetrap ") || C(b, this.target)
					? !1
...
window.AvsAn = (function () {
	...
	function fill(prefix, node, dict) {
		...
		for (var i = 0; i < n[2]; i++) dict = fill(prefix + dict[0], (node[dict[0]] = {}), dict.substr(1));
		return dict;
	}
	fill("", root, dict);
	return {
