...
$(document).on(":passageend", function () {
	...
	try {
		...
		for (var name in wpRec) {
			var rec = wpRec[name];
			var grec = Perflog.globalRec[name];
			if (!grec) {
				Perflog.globalRec[name] = rec;
			} else {
				grec.n += rec.n;
...
Perflog.logWidgetEnd = function (widgetName) {
	...
	wpStack.pop();
	Perflog.logWidgetTime(widgetName, time - perflog.t0, perflog.i);
	if (Perflog.self) {
